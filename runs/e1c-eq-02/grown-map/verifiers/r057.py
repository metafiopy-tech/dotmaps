#!/usr/bin/env python3
"""Grown check r057 — compiled from a banked rule, template-generated.

Rule: Writing a JSON array with a unicode escape (\u00e9) in a field to target_items.json persists and reads back with the literal escape sequence \u00e9 intact (not decoded to é)
...
try:
    ...
    print(json.dumps({"dot": 'r057', "pass": False,
                      "evidence": "file target_items.json missing"}))
    sys.exit(1)

predicate, value = 'contains', 'Caf\\u00e9'
...
print(json.dumps({"dot": 'r057', "pass": ok,
