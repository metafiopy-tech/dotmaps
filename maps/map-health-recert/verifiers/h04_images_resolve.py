...
if status != 200:
    emit("h04", False, f"homepage -> {status}")
...
if len(srcs) < min_images:
    emit("h04", False, f"found {len(srcs)} images, expected >= {min_images}")
bad = []
for s in srcs:
    ...
    if st != 200:
        bad.append(f"{s}->{st}")
ok = not bad
emit("h04", ok, f"{len(srcs)} images; " + ("all resolve" if ok else f"broken: {', '.join(bad[:5])}"))
