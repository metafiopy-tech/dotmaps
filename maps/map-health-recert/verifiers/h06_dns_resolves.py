...
if not domain:
    emit("h06", True, "no custom domain configured (dot vacuously satisfied)")
host = urlparse(domain if "//" in domain else "//" + domain).hostname or domain
try:
    ip = socket.gethostbyname(host)
except Exception as e:
    emit("h06", False, f"DNS for {host} did not resolve: {e}")
status, _, _ = fetch((domain if "//" in domain else "https://" + domain).rstrip("/") + "/")
ok = status == 200
emit("h06", ok, f"{host} -> {ip}, HTTPS root -> {status}")
