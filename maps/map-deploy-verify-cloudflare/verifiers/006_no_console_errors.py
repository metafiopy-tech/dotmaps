...
try:
    ...
    emit("006", False, "playwright not installed (run in this map's Docker image)", error=True)
errors = []
with sync_playwright() as p:
    ...
    b.close()
ok = not errors
emit("006", ok, "no console errors" if ok else f"{len(errors)} console errors: {errors[:3]}")
