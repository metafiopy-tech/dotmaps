...
if not shutil.which("lighthouse"):
    emit("009", False, "lighthouse CLI not installed (run in this map's Docker image)", error=True)
...
try:
    ...
    emit("009", False, f"lighthouse produced no report ({e}); stderr: {tail}", error=True)
ok = score >= threshold
emit("009", ok, f"lighthouse performance {score} (threshold {threshold})")
